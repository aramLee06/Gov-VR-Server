...
module.exports = function(obj) {
	let buffer_len = 2 + (38 * obj.players.length)
	...
	for(var i = 0, len = obj.players.length; i < len; i++) {
		...
		buffer.writeFloatLE(p.current.x, offset + 26)
		buffer.writeFloatLE(p.current.y, offset + 30)
		buffer.writeFloatLE(p.current.z, offset + 34)
	}
